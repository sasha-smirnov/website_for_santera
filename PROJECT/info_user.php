<?php
    include "validation/history_db.php";
...
<header class="header">
    ...
    <form class="form form_login" method="post" action="validation/change_info.php">
        <h2 class="form__text form__text_login">Личная информация</h2>
        <input name="email" class="form__field-text form__field-text_login" placeholder="Email" type="text"
            size="40" required minLength="2" id="login" maxLength="40" onChange="handleUserEmail" value="<?=$_COOKIE['email']?>" />
        <span id="username-error" class="username-error form__input-error"></span>
        <input name="name" class="form__field-text form__field-text_login" placeholder="Имя" type="text"
            size="40" required minLength="2" id="name" maxLength="40" onChange="handleUserEmail" value="<?=$_COOKIE['user']?>"/>
        <input name="number" class="form__field-text form__field-text_login" placeholder="+7 (___)___-__-__" type="text"
            size="40" required minLength="2" id="number" maxLength="40" onChange="handleUserEmail" value="<?=$_COOKIE['number']?>" />
        <input name="city" class="form__field-text form__field-text_login" placeholder="Город" type="text"
            size="40" required minLength="2" id="city" maxLength="40" onChange="handleUserEmail" value="<?=$_COOKIE['city']?>"/>
        ...
        <button type="submit" class="form__save-button form__save-button_login">Изменить</button>
    </form>
    <div class="download__container">
        <div class="download__catalog info__user">
        ...
        <div class="info_user-telephone info__user">   
            <a href="tel:+74951206677" class="info_user__contacts_phone">8 (495) 120-66-77</a>
        </div>
    </div>
    <div class="order_history">
        <h2 class="order_history__text">История заказов</h2>
        <div class="row">
            <div class="col-md-12">
                <?php
                ...
                <div class="history-articles">
                    <div class="history-article"><?php echo $purchases['description']; ?></div>
                    <div class="history-article"><?php echo $purchases['price']; ?></div>
                    <div class="history-article"><?php echo $purchases['date']; ?></div>
    ...
    <form action="validation/delete_user.php">
        <button class="delete__user">Удалить аккаунт</button>
    </form>
